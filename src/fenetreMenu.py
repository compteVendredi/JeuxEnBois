...
class FenetreMenu():
    def __init__(self, jouer):
        ...
        self.radio_jcj = Radiobutton(self.fenetre, text="Joueur contre joueur", command=self.choixExclusif, variable=self.var_mode, value="JcJ")
        self.radio_jcj.grid(row=1, column=0, padx=5, pady=5, sticky="w")
        self.radio_jco = Radiobutton(self.fenetre, text="Joueur contre ordinateur"\
                                     , command=self.choixExclusif, variable=self.var_mode, value="JcO")
        ...
        self.radio_ia1 = Radiobutton(self.fenetre, text="IA BFS (traditionnel)", state=DISABLED, variable=self.var_ia, value="IA_BFS")
        self.radio_ia1.grid(row=4, column=0, padx=20, pady=5, sticky="w")
        self.radio_ia2 = Radiobutton(self.fenetre, text="IA MLP (réseau de neurones)", state=DISABLED, variable=self.var_ia, value="IA_MLP")
    ...
    def choixExclusif(self):
        if self.var_mode.get() == "JcO":
            self.radio_ia1.configure(state="normal")
            self.radio_ia2.configure(state="normal")
        else:
            self.radio_ia1.configure(state="disabled")
            self.radio_ia2.configure(state="disabled")                  
